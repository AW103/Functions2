...
// }
// const postTaxPrices = prices.map(addTax);
...
// CODE HERE
const totalPopulation = populations.reduce((a, c) => a + c, 0);
...
// const myStrongest = monstersInYourPocket.filter(mons => {
//   let cp = mons.CP;
//   if (cp > 200) {
//   return cp;
//   }
...
const orderTotal = orders.map((arr) => (parseFloat((arr.price * (1 + arr.tax)).toFixed(2))))
...
// const findBob = purchases.filter(purch => purch.owner === "Bob")
// console.log(findBob);

// const bobsPrices = findBob.map((bob) => bob.price)
// console.log(bobsPrices);

// const bobsTotal = bobsPrices.reduce((a,c) => a + c, 0);
// console.log(bobsTotal);

const allInOneBob = purchases.filter(purch => purch.owner === "Bob").map(purch => purch.price).reduce((a,c) => a + c)
console.log(allInOneBob);

// const reducedBobTotal = purchases.reduce((acc, purchase) => {
