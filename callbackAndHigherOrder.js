...
const last = (arr, fn) => {fn(arr[arr.length-1])
}
...
// }
const contains = (arr, name, fn) => {
  arr.filter(arrIndex => { arrIndex === name ? fn(true) : fn(false)})
}
...
// const uniq = (arr, fn) => {
//  let duplicates = arr.filter((elem, index) =>
...
const each = (names, fn) => {
  names.forEach((elem, index) => fn(elem, index))
}
...
const getUserById = (arr, userId, fn) => {
arr.forEach((user) => {user.id === userId ? fn(user) : "Invalid"})
}
